...
public class Cli : IDisposable
{
    ...
    public async Task PlayAsync()
    {
        ...
        GameState response = await PostAsync<GameState, ConnectPlayerRequest>($"game/{_gameId}/connect", new(_player));
        ...
        while (true)
        {
            ...
            // POLLING
            while (response.Turn != _player || response.ConnectedPlayers != Game.XO)
            {
                response = await _client.GetFromJsonAsync<GameState>($"game/{_gameId}") ?? throw new HttpRequestException();

                // Информационное сообщение        
                string message = $"Ожидание {(response.ConnectedPlayers != Game.XO ? "подключения" : "хода")} {(_player == Game.X ? 'O' : 'X')}...";
    ...
    private bool PrintGameBoardAndCheckWin(GameState Game)
    {
        Console.Clear();
        string result = "";
        for (int y = 0; y < 9; y += 3)
    ...
    public async Task JoinToGameAsync()
    {
        ...
        while (true)
        {
            ...
            {
                GameState response = await _client.GetFromJsonAsync<GameState>($"game/{matchingGuids[0]}") ?? throw new HttpRequestException();
                if (response.ConnectedPlayers != Game.XO)
                {
                    Console.WriteLine(matchingGuids[0].ToString()[input.Length..]);
                    _player = response.ConnectedPlayers ^ Game.XO;
